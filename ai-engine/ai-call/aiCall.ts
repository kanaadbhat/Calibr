...
const extractJsonFromResponse = (text : any) => {    
...
export const getGeminiResponse = async (request : any , json : boolean) => {
    try {
        let rawResponse = await apiCall((json == true) ? "JSON format" + request : request);
        if (!json) {
            return rawResponse;
        }
        ...
        try {
            ...
            try {
                ...
                throw new Error(`Could not parse JSON after multiple attempts. Original error: ${parseError.message}`);
            }
        }
